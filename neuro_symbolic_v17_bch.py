...
NeuroSymbolic V17-CUDA — AND Instruction Distribution Edition
  + BCH PAYMENT GATE (one-time licence, verified via Blockchair API)
===============================================================================

NEW IN THIS VERSION: BCH PAYMENT GATE
──────────────────────────────────────
  A one-time BCH payment to a configurable address is required before the
  engine can be trained or used.

  Payment is checked:
    1. At CLI startup  (before any engine work begins)
    2. Via a dedicated "License" tab in the Gradio GUI

  Verification uses the Blockchair public blockchain API (no API key needed
  for normal usage).  Once verified, a local access-token file is written so
  subsequent runs do not require a network request.

  Configure in bch_paygate.py (or inline below under PAYMENT GATE CONFIG):
    BCH_PAYMENT_ADDRESS   — your BCH address
    REQUIRED_BCH          — minimum amount to accept  (e.g. 0.01)
    LICENSE_DURATION_DAYS — 0 = lifetime, N = expires after N days
...
# ════════════════════════════════════════════════════════════════════════════
# PAYMENT GATE — import and inline config
# ════════════════════════════════════════════════════════════════════════════
# The gate lives in bch_paygate.py (same directory).  All constants can also
# be overridden here after the import if you don't want a separate file.

try:
    from bch_paygate import (
        check_payment_cli,
        build_gradio_tab,
        get_session_timer,
        SessionTimer,
    ...
    _PAYGATE_AVAILABLE = True
except ImportError:
    _PAYGATE_AVAILABLE = False
    SESSION_TIMEOUT_SECONDS = 600
    print(
        "[WARNING] bch_paygate.py not found — payment gate is DISABLED.\n"
        "          Place bch_paygate.py in the same directory to enable it."
    )
    # Stubs so the rest of the code compiles cleanly
    def check_payment_cli(silent=False): return True          # noqa: E704
    def build_gradio_tab(gui): pass                           # noqa: E704

    class SessionTimer:                                       # noqa: E302
        def start(self): pass
        def expired(self): return False
        def seconds_remaining(self): return float(SESSION_TIMEOUT_SECONDS)
        def progress(self): return 0.0
        def status_line(self): return "Session active."

    _stub_timer = SessionTimer()
    def get_session_timer(): return _stub_timer               # noqa: E302

    BCH_PAYMENT_ADDRESS = "bitcoincash:qp63uahgrxged4y5qtnysne3t6xekpd35g3q48yfkh"
    REQUIRED_BCH        = 0.000216
...
# ════════════════════════════════════════════════════════════════════════════
# SECTION 14 — GRADIO GUI  (+ License tab)
# ════════════════════════════════════════════════════════════════════════════
...
class V17GUI:
    ...
    def generate_text(self, sentences, tokens, seed_text, instruction_text, and_weight, temperature):
        if not self.engine or not self.engine.walker:
            return "Engine not initialised.", "", "", "Engine not initialised."

        timer = get_session_timer()
        if timer.expired():
            return (
                "⏱  SESSION EXPIRED\n\nYour 10-minute session has ended.\n"
                "Please restart the application to begin a new session.",
                "", "",
                "⏱  Session EXPIRED — restart to continue.",
            )
        ...
        return text, trace_text, step_report, timer.status_line()

    def session_status(self):
        return get_session_timer().status_line()

    def session_progress(self):
        """Returns 0–100 int for gr.Slider / progress indicator."""
        return int(get_session_timer().progress() * 100)
...
def launch_gui():
    gui = V17GUI()
    # NOTE: session timer is NOT started here.
    # It is started inside bch_paygate._write_token() / check_payment_cli()
    # — i.e. only after payment is confirmed.

    # ── Helper Gradio fn: return timer start_ts (or -1 if not yet started) ──
    def _session_start_ts() -> float:
        t = get_session_timer()
        return t._start_ts if t._start_ts is not None else -1.0

    # ── JS: polls Python for the payment-confirmed timestamp, then counts down ──
    _TIMEOUT_JS = f"""
    <script>
    (function() {{
        const TIMEOUT_MS  = {SESSION_TIMEOUT_SECONDS * 1000};
        let   sessionStart = null;   // null = not paid yet
        let   warned       = false;

        // Ask the Gradio backend whether payment has been confirmed yet.
        // We use the hidden textbox trick: a gr.Number named "session_ts_out"
        // is updated by a gr.Timer every 3 s.  We read its value from the DOM.
        function readStartTs() {{
            // Find the hidden input with id "session-ts-hidden"
            const el = document.getElementById("session-ts-hidden");
            if (!el) return null;
        ...
        function tick() {{
            const ts = readStartTs();

            if (ts === null) {{
                // Not paid yet — show waiting state
                const cd = document.getElementById("v17-countdown");
                if (cd) {{
                    cd.textContent = "💳  Awaiting payment — timer paused";
                    cd.style.color = "#94a3b8";
                }}
                // Keep Generate button locked until paid
                const btns = document.querySelectorAll("button");
                btns.forEach(b => {{
                    if (b.textContent.trim() === "Generate" || b.textContent.trim() === "⏱  Expired") {{
                        b.style.opacity       = "0.25";
                        b.style.pointerEvents = "none";
                        b.style.cursor        = "not-allowed";
                        b.title               = "Complete payment first";
                    }}
                }});
            ...
            // Payment confirmed — start counting down from ts
            if (sessionStart === null) {{
                sessionStart = ts * 1000;  // convert Python time() (seconds) → ms
                // Unlock the button once payment is confirmed
                const btns = document.querySelectorAll("button");
                btns.forEach(b => {{
            ...
            const progress  = Math.min(1.0, elapsed / TIMEOUT_MS);
            ...
            if (cd) {{
                ...
                cd.textContent = remaining > 0
                    ...
                    : "⏱  Session expired — restart to continue";
                cd.style.color = remaining < 60000 ? "#e53e3e"
                               : remaining < 180000 ? "#d97706"
                               : "#22c55e";
            }}

            // Fade + disable Generate button
            const btns2 = document.querySelectorAll("button");
            let genBtn = null;
            btns2.forEach(b => {{ if (b.textContent.trim() === "Generate") genBtn = b; }});
            if (genBtn) {{
                if (remaining > 0) {{
                    const opacity = 1.0 - (progress * 0.85);
                    genBtn.style.opacity    = opacity.toFixed(3);
                    genBtn.style.transition = "opacity 2s ease";
                    genBtn.style.cursor     = remaining < 30000 ? "not-allowed" : "pointer";
                    if (remaining < 60000 && !warned) {{
                        warned = true;
                        genBtn.title = "⚠  Less than 1 minute remaining!";
                    }}
                }} else {{
                    genBtn.style.opacity       = "0.08";
                    genBtn.style.pointerEvents = "none";
                    genBtn.style.cursor        = "not-allowed";
                    genBtn.title               = "Session expired — restart to continue";
                    genBtn.textContent         = "⏱  Expired";
                }}
            }}

            setTimeout(tick, remaining > 0 ? 1000 : 5000);
        }}
    ...
    with gr.Blocks(
        title="NeuroSymbolic V17 CUDA + PDN + CoT + AND",
        head=_TIMEOUT_JS,
    ) as app:
        gr.Markdown(
        ...
        # Hidden numeric field that carries the session start timestamp to JS.
        # Value = -1 means "not paid yet"; value = Unix timestamp means "paid, count from here".
        session_ts_out = gr.Number(
            value=-1,
            visible=False,
            elem_id="session-ts-hidden",
            label="session_ts",
        )
        ...
        with gr.Tab("Generate"):
            ...
            with gr.Row():
                gr.HTML(
                    '<div id="v17-countdown" style="'
                    'font-family:monospace;font-size:0.95rem;padding:6px 12px;'
                    'border-radius:6px;background:#1a1a2e;color:#94a3b8;'
                    'display:inline-block;min-width:300px;text-align:center;">'
                    '💳  Awaiting payment — timer paused'
                    '</div>'
                )
                session_status_out = gr.Textbox(
                    label="Session Status",
                    value="Awaiting payment — complete payment in the License tab first.",
                    interactive=False,
                    scale=3,
                )
            ...
            gen_btn   = gr.Button("Generate", variant="primary", elem_id="v17-gen-btn")
            ...
            gen_btn.click(
                ...
                outputs = [gen_out, cot_out, step_out, session_status_out],
            )

            # Refresh server-side status text on slider change
            for component in [sentences, tokens, and_weight, temperature]:
                component.change(
                    lambda: get_session_timer().status_line(),
        ...
        # ── BCH License tab ──────────────────────────────────────────────
        with gr.Tab("💳 License / BCH Payment"):
            build_gradio_tab(gui)

        # ── Polling timer: pushes session_ts to JS every 2 s ─────────────
        # gr.Timer fires a server-side callback that updates the hidden field.
        # JS reads it from the DOM to know when payment was confirmed.
        poll_timer = gr.Timer(value=2, active=True)
        poll_timer.tick(fn=_session_start_ts, outputs=session_ts_out)

    app.launch()


# ════════════════════════════════════════════════════════════════════════════
# ENTRY POINT — payment gate runs BEFORE anything else
# ════════════════════════════════════════════════════════════════════════════

if __name__ == "__main__":
    # ── 1. Payment gate (CLI only; GUI has its own License tab) ──────────
    import sys as _sys
    _is_gui_launch = ("--gui" in _sys.argv or len(_sys.argv) == 1)

    if not _is_gui_launch:
        if not check_payment_cli():
            _sys.exit(1)
    ...
    if args.gui or not args.corpus:
        launch_gui()   # timer started inside bch_paygate when payment confirmed
        exit(0)

    # ── 3. CLI session timer ─────────────────────────────────────────────
    # Timer was already started by check_payment_cli() above (on payment confirm).
    _timer = get_session_timer()
    print(f"[Session] {SESSION_TIMEOUT_SECONDS // 60}-minute session is running.")
    ...
    engine.save_cache("v17_model.pkl")

    if _timer.expired():
        print("\n[Session] ⏱  Timeout reached — training took the full session window.")
        print("          Re-run the script to start a new session.")
        exit(0)
    ...
    print(f"\n{_timer.status_line()}")
    if _timer.expired():
        print("[Session] ⏱  Session has now expired. Re-run to start a new session.")
