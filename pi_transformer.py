...
if __name__ == "__main__":
    pipe, pre = build_pipeline_from_preset(
        "imdb", locked=False, ngram_n=3,
        preprocessor_kw=dict(boundaryquota=8, max_per_split=1000),
    )
    trainer = AutomorphismTrainer(pipe, n_cands=100, lr=5e-4)
    trainer.warmup(steps=50, log_every=25)
    trainer.run(n_steps=40, prompt="tell me about yourself",
                n_words=20, seed=42, log_every=10, patience=8)
    trainer.report()
    print("Fixed-point checks:", trainer.check_fixed_point(n_samples=32, tol=0.08))

    p_test = _norm(torch.rand(20, dtype=torch.float64))
    print("\n── Rule audit (random p) ──")
    for k, v in R.audit(p_test).items():
    ...
    while True:
        ...
        print(pipe.generate_text(prompt, n_words=200, seed=42))
        print()
