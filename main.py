...
# -*- coding: utf-8 -*-
"""
NeuroSymbolic V18-RP-ANISO-RIPPLE-SPAGHETTI-CARDAN + MINI-THEBAULT
===============================================================================
Mini-Thébault three-point curvature correction applied inside every submodule
signal producer before signals enter the spaghetti router.

Thébault geometry: for each element i, the two neighbours (i-1, i+1) form the
base of a triangle whose apex is element i. The midpoint of the base is the
Thébault interpolant. We blend element i toward that midpoint proportional to
local curvature (|apex_delta| / max), then z-normalise the result.

Applied at 8 sites:
  1. RippleShiftEngine.compute           gamma=0.15
  2. SentenceOOITracker.ooi_affinity     gamma=0.12
  3. SentenceOOITracker.inter_candidate_repulsion  gamma=0.12
  4. RPCrossSynapticNeuronSum.synaptic_sum         gamma=0.10
  5. RPCrossSynapticNeuronSum.transitive_bonus      gamma=0.10
  6. IsomorphicSyntaxStacker.syntax_echo_bonus      gamma=0.13
  7. RPCoTReasoningEngine.active_bonus              gamma=0.11
  8. SpaghettiMixer.blend                           gamma=0.10

Also includes mirrored sawtooth prob clipping in walk_probs (fixes the
silent no-op torch.clip call) and all prior V18-CARDAN machinery unchanged.
"""

# ============================================================
# FILE 2: AI.py  (TokenCycle layer)
# ============================================================
import math
import re
import time
...
PHASE_COLORS = {
    "INPUT": "\033[94m", "FWD": "\033[93m", "REV": "\033[96m",
    "AND": "\033[92m", "EMIT": "\033[97m"
}
RESET = "\033[0m"
...
class MachineChoice:
    ...
    def register(self, name: str, condition: Callable, handler: Callable):
...
class WeightedMachineChoice(MachineChoice):
    ...
    def register(self, name, condition, handler, weight: float = 1.0):
        super().register(name, condition, handler)
        self._weights[name] = float(weight)

    def select(self, tokens: List[str]) -> Tuple[str, str]:
        matches = [
            (name, condition, handler, self._weights.get(name, 1.0))
            for name, condition, handler in self._options if condition(tokens)
        ]
        if not matches:
            raise ValueError(f"XOR violation: no option matched tokens {tokens[:4]}…")
        weights = [m[3] for m in matches]
        idx = random.choices(range(len(matches)), weights=weights, k=1)[0]
        ...
        while attempt < MachineChoice.MAX_RETRIES:
            ...
            try:
                ...
                if attempt == MachineChoice.MAX_RETRIES:
                    raise RuntimeError(
                        f"XOR failed after {MachineChoice.MAX_RETRIES} attempts: {exc}"
                    ) from exc
...
class TokenCycleEmitter:
    def __init__(self, engine=None, mirror_alpha: float = 0.35,
                 cardan_logit_weight: float = 8.0, emit_delay_ms: float = 0.0):
        self.engine = engine
    ...
    def _build_controller(self) -> CombinatorialController:
        ...
        ctrl.register_option(
            "cognitive_token",
            lambda toks: len(toks) >= 1 and toks[-1] not in PUNCT_SET
                         and any(t.startswith("[") for t in toks[-3:]),
    ...
    def cycle(self, instruction: str = "You are a computational algorithm.",
              seed: str = "", n_sentences: int = 4, tokens_per_sent: int = 40,
              and_weight: float = 0.9, temperature: float = 2.0) -> Generator:
        self._step = 0
        self._start_ts = time.perf_counter()
        if self.engine is not None:
            yield from self._cycle_real(
                instruction, seed, n_sentences, tokens_per_sent, and_weight, temperature)
        else:
    ...
    def _cycle_real(self, instruction, seed, n_sentences, tokens_per_sent, and_weight, temperature):
        ...
        try:
            result = eng.generate(
                seedtext=seed, instructiontext=instruction,
                numsentences=n_sentences, tokenspersent=tokens_per_sent,
                andweight=and_weight, temperature=temperature, returntraces=True,
            )
        ...
        if hasattr(eng, "walker") and eng.walker is not None:
            traces = list(getattr(eng.walker, "steptraces",
                                  getattr(eng.walker, "_step_traces", [])))
        trace_idx = 0
        for sent_idx, sent_text in enumerate(sentences):
            ...
            for tok_pos, tok in enumerate(tokens):
                ...
                yield CycleTokenEvent(
                    self._step, sent_idx, tok, bound, cardan_orbit,
                    self.mirror_alpha, spaghetti_norm, elapsed, tok in PUNCT_SET, phase)
                self._step += 1
            yield CycleSentenceEvent(
                sent_idx, sent_text, tokens,
                cot_trace=cot_text if sent_idx == 0 else "",
                avg_cardan_orbit=orbit_sum / max(len(tokens), 1))
    ...
    def _cycle_demo(self, instruction, seed, n_sentences, tokens_per_sent):
        ...
        vocab = (corpus_vocab_from_text(corpus_text)
                 or ["algorithm", "process", "token", "cycle", "output", "signal"])
        rng = random.Random(42)
        windows = list(_bidirectional_windows(
            corpus_tokens, tokens_per_sent, max(1, tokens_per_sent // 2)))
        if not windows:
            windows = [vocab]
        for sent_idx in range(n_sentences):
            ...
            for i, tok in enumerate(window[:max(6, min(tokens_per_sent, len(window)))]):
                tokens.append(tok if i % 2 == 0 else rng.choice(vocab))
            orbit_sum = 0.0
            for tok_pos, tok in enumerate(tokens):
                ...
                self._step += 1
            yield CycleSentenceEvent(
                sent_idx, _detokenize(tokens), tokens,
                cot_trace=f"[CORPUS DEMO] source_len={len(corpus_tokens)}",
                prop_stmts=[f"⟨ {tokens[0]} → {tokens[min(1, len(tokens)-1)]} → {tokens[-1]} ⟩"],
                avg_cardan_orbit=orbit_sum / max(len(tokens), 1))

    @classmethod
    def demo(cls, mirror_alpha: float = 0.35,
             cardan_logit_weight: float = 8.0) -> "TokenCycleEmitter":
        return cls(engine=None, mirror_alpha=mirror_alpha,
                   cardan_logit_weight=cardan_logit_weight)


def render_cycle_stream(emitter: TokenCycleEmitter,
                        instruction: str = "You are a computational algorithm.",
                        seed: str = "", n_sentences: int = 3, tokens_per_sent: int = 40,
                        and_weight: float = 0.9, temperature: float = 2.0,
                        verbose: bool = True, show_bound: bool = False) -> str:
    print("\n" + "═" * 70)
    ...
    for event in emitter.cycle(instruction=instruction, seed=seed, n_sentences=n_sentences,
                                tokens_per_sent=tokens_per_sent, and_weight=and_weight,
        ...
        if isinstance(event, CycleTokenEvent):
            ...
            if verbose:
                print(f"{phase_col}{event.token}{RESET}\033[90m{orbit_sym}\033[0m",
                      end=" ", flush=True)
            else:
...
def stream_cycle_text(emitter: TokenCycleEmitter,
                      instruction: str = "You are a computational algorithm.",
                      seed: str = "", n_sentences: int = 3, tokens_per_sent: int = 40,
                      and_weight: float = 0.9, temperature: float = 2.0,
                      orbit_sort: bool = False):
    token_events = []
    for event in emitter.cycle(instruction=instruction, seed=seed, n_sentences=n_sentences,
                                tokens_per_sent=tokens_per_sent, and_weight=and_weight,
                                temperature=temperature):
...
def gui_generate_cycle(seed, instruction, nsents, tokssent, andw, temp,
                       showtr, artimage, _engine_ref=None):
    eng = _engine_ref
    if eng is None:
        try:
            import __main__
    ...
    latest = ""
    for partial in stream_cycle_text(
            emitter,
            instruction=instruction or "You are a computational algorithm.",
            seed=seed or "", n_sentences=int(nsents), tokens_per_sent=int(tokssent),
            and_weight=float(andw), temperature=float(temp), orbit_sort=True):
        latest = partial
    return latest, "", "", ""


def attach_cycle_to_engine(engine, mirror_alpha: float = 0.35,
                           cardan_logit_weight: float = 8.0) -> TokenCycleEmitter:
    emitter = TokenCycleEmitter(engine=engine, mirror_alpha=mirror_alpha,
                                cardan_logit_weight=cardan_logit_weight)
    engine.cycle_emitter = emitter
    print(f"[TokenCycle] Emitter attached to {type(engine).__name__} "
          f"mirror_α={mirror_alpha} cardan_lw={cardan_logit_weight}")
    return emitter


# ============================================================
# FILE 1: v18_token_cycle.py  (Core engine)
# ============================================================

import re as _re, math as _math, random as _random, unicodedata, pickle, hashlib
import argparse, struct
from dataclasses import dataclass as _dc, field as _field
from pathlib import Path as _Path
from typing import List as _L, Dict as _D, Tuple as _T, Set as _S, Optional as _O
import torch
...
# ---------------------------------------------------------------------------
# Global autonomic vessel carrier
# ---------------------------------------------------------------------------
LATEST_AUTONOMIC_VAL = 1.0


# ---------------------------------------------------------------------------
# ██  MINI-THÉBAULT  ─ core utility (applied in 8 submodules)
# ---------------------------------------------------------------------------
def mini_thebault(signal: torch.Tensor, gamma: float = 0.15) -> torch.Tensor:
    """
    Thébault-inspired three-point curvature correction for a 1-D signal vector.

    For each element i the two neighbours form the base of a triangle whose
    apex is element i.  We compute the chord midpoint (Thébault interpolant)
    and blend element i toward it proportional to local normalised curvature.

    Boundary elements use edge-reflection padding.
    Output is z-normalised to preserve relative scale.
    """
    if signal.shape[0] < 3:
        return signal
    s = signal.float()
    padded     = torch.cat([s[:1], s, s[-1:]])   # reflect edges
    left       = padded[:-2]                       # neighbours i-1
    right      = padded[2:]                        # neighbours i+1
    midpoint   = (left + right) / 2.0
    apex_delta = s - midpoint
    curvature  = apex_delta.abs() / apex_delta.abs().max().clamp(min=1e-8)
    adaptive_g = gamma * curvature
    blended    = s - adaptive_g * apex_delta
    std = blended.std()
    if std.item() > 1e-8:
...
# ---------------------------------------------------------------------------
# ██  MIRRORED SAWTOOTH CLIP
# ---------------------------------------------------------------------------
def mirrored_sawtooth_clip(probs: torch.Tensor, period: float = 0.1) -> torch.Tensor:
    """
    Clips probability wave using a mirrored sawtooth (triangle wave) envelope.
    Values fold within [0, period] via abs(mod) reflection, then renormalise.
    Replaces the silent no-op torch.clip at the end of walk_probs.
    """
    half    = period / 2.0
    folded  = probs % period
    mirrored = torch.where(folded <= half, folded, period - folded)
    mirrored = (mirrored / half).clamp(min=1e-12)
    return mirrored / mirrored.sum()
...
def maj_gate(x: torch.Tensor, y: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
    ...
    maj_mask = ((bx * by) + (bx * bz) + (by * bz) >= 2).float()
    return maj_mask * (x + y + z) / 3.0 + (1.0 - maj_mask) * (x + y + z).clamp(min=0) / 3.0


# ---------------------------------------------------------------------------
# ██  BOTTEMA PROBABILITY BRIDGE
# ---------------------------------------------------------------------------
def apply_bottema_probability_bridge(cands, probs, gamma=0.20):
    ...
    if is_tensor:
        Y = probs.clone().float(); device = probs.device; orig_dtype = probs.dtype
    else:
        Y = torch.tensor(probs, dtype=torch.float32); device = Y.device; orig_dtype = torch.float32
    X = torch.linspace(0.0, 1.0, len(probs), device=device)
    idx_max = torch.argmax(Y); idx_min = torch.argmin(Y)
    A_x, A_y = X[idx_max], Y[idx_max]
    B_x, B_y = X[idx_min], Y[idx_min]
    dx_AB = (B_x - A_x) / 2.0; dy_AB = (B_y - A_y) / 2.0
...
CARDAN_APERTURE_K   = 64
MIN_CARDAN_CANDS    = 4
CARDAN_LOGIT_WEIGHT = 8.0
...
def mobius_cross_shift(a: torch.Tensor, b: torch.Tensor,
    ...
    a = a.clamp(-20.0, 20.0); b = b.clamp(-20.0, 20.0)
    ta = torch.tanh(a * 0.1);  tb = torch.tanh(b * 0.1)
...
class SpaghettiMixer:
    ...
    def blend(self, C: int,
        ...
        weighted = [
            s.sign * s.weight * (s.signal if s.signal.shape[0] == C
        ...
        if len(weighted) >= 3:
            ...
            acc = weighted[0]
        # ── MINI-THÉBAULT site 8: SpaghettiMixer.blend ──────────────────────
        normed = mini_thebault(layer_norm_array(acc), gamma=0.10)
        return normed / max(SPAGHETTI_MIXER_TEMP, 1e-6)


class CrossTangle:
    def __init__(self, mixer_i: int, mixer_j: int, coupling: float = SPAGHETTI_COUPLING):
        self.i = mixer_i; self.j = mixer_j; self.coupling = coupling

    def apply(self, outputs: List[torch.Tensor]) -> List[torch.Tensor]:
        a = outputs[self.i]; b = outputs[self.j]
...
class SpaghettiRouter:
    ...
    def __init__(self, C: int, device: torch.device = DEVICE):
        self.C       = C
        self.device  = device
        self._mixers = [SpaghettiMixer(name, device) for name in ('MixerA', 'MixerB', 'MixerC')]
...
class CardanGrilleIsomorphism:
    def __init__(self, vocab: List[str],
        ...
        self.vocab = vocab; self.aperture_k = aperture_k; self.min_cands = min_cands
        self._tok2idx = {t: i for i, t in enumerate(vocab)}
        V = len(vocab)
        self._G = max(1, math.ceil(math.sqrt(V)))
        ...
        scored = sorted(
            [(head_score.get(t, raw_freq.get(t, 0.0)), i, t) for i, t in enumerate(vocab)],
            reverse=True)
        base_positions = [item[1] for item in scored[:aperture_k]]

        def idx_to_rc(idx):   return divmod(idx, self._G)
        def rc_to_idx(r, c):  return min(r * self._G + c, V - 1)
        def rotate_90(r, c, G): return c, G - 1 - r
        ...
        for pos in base_positions:
            ...
            r1, c1 = rotate_90(r, c, G);   self._grilles[1].add(rc_to_idx(r1, c1))
            r2, c2 = rotate_90(r1, c1, G); self._grilles[2].add(rc_to_idx(r2, c2))
            r3, c3 = rotate_90(r2, c2, G); self._grilles[3].add(rc_to_idx(r3, c3))

        self._grille_toks: List[Set[str]] = [
            {vocab[i] for i in grille if i < V} for grille in self._grilles]
        sizes = [len(g) for g in self._grille_toks]
        print(f"[Cardan] Grid {G}×{G}  aperture_k={aperture_k}  rotation set sizes: {sizes}")

    def filter(self, cands: List[str], base_probs: torch.Tensor,
               orbit: int) -> Tuple[List[str], torch.Tensor, torch.Tensor]:
        device = base_probs.device; C = len(cands)
...
# ---------------------------------------------------------------------------
# ██  MIRRORED INSTRUCTION DISTRIBUTION
# ---------------------------------------------------------------------------
class MirroredInstructionDistribution:
    def __init__(self, forward_dist: 'RPInstructionDistribution',
        ...
        self.fwd = forward_dist; self.alpha = alpha
    ...
    def set_instruction(self, instruction_text: str):
        ...
        if not mir_toks or self.fwd.base_dist_t is None:
            self.mirror_dist_t = self.fwd.base_dist_t; return

        geo = self.fwd.geo; lm = self.fwd.lm; kernels = self.fwd.kernels
        V   = len(lm.vocab); N = len(mir_toks); decay = self.fwd.recency_decay
        freq: Dict[str, float] = {}
        ...
        if geo._rho_t is not None and geo._rho_t.shape[0] == V:
            for tok, w in freq.items():
                if tok not in geo._vecs: continue
        ...
        print(f"[Mirror] Reversed {len(fwd_toks)}→{len(mir_toks)} tokens  "
              f"ρ={self.mirror_centroid_rho:.3f}  "
    ...
    def distribution(self, cands: List[str], gen_tokens: List[str],
        ...
        if self.mirror_dist_t is None or len(cands) == 0:
            return p_fwd
        cand_idx  = torch.tensor([lm_tok2idx.get(c, 0) for c in cands],
                                  dtype=torch.long, device=self.device)
        p_mir_raw = self.mirror_dist_t[cand_idx].clamp(min=1e-12)
    ...
    def mirror_signal(self, cands: List[str], lm_tok2idx: Dict[str, int]) -> torch.Tensor:
...
# ---------------------------------------------------------------------------
# ██  OOI TRACKER  (mini-thébault sites 2 & 3)
# ---------------------------------------------------------------------------
@dataclass
class OOIEntry:
...
class SentenceOOITracker:
    ...
    def ooi_affinity(self, c_rho, c_theta, c_sigma) -> torch.Tensor:
        ...
        raw = agg / len(self._ooi)
        # ── MINI-THÉBAULT site 2 ────────────────────────────────────────────
        return mini_thebault(raw, gamma=0.12)
    ...
    def inter_candidate_repulsion(self, c_rho, c_theta, c_sigma, prob_vec,
        ...
        # ── MINI-THÉBAULT site 3 ────────────────────────────────────────────
        return mini_thebault(raw, gamma=0.12)


# ---------------------------------------------------------------------------
# ██  STUB DIRECTIVE + RECOGNISER + RIPPLE ENGINE  (mini-thébault site 1)
# ---------------------------------------------------------------------------
@dataclass
class StubDirective:
    stub_idx: int; stub_rho: float; stub_theta: float; stub_sigma: float; directive: float
...
class InstructionStubRecogniser:
    def __init__(self, stub_library, aniso_kernel: AnisoDirKernel,
                 ...
                 device: torch.device = DEVICE):
        self.stubs = stub_library; self.kernel = aniso_kernel
        self.k_stubs = k_stubs; self.scale = recogniser_scale; self.device = device
    ...
    def set_instruction(self, instr_rho: float, instr_theta: float, instr_sigma: float):
        self._instr_rho = instr_rho; self._instr_theta = instr_theta
        self._instr_sigma = instr_sigma; self._instr_set = True

    @torch.no_grad()
    def locate(self) -> List[StubDirective]:
        if not self._instr_set or not self.stubs._stub_list: return []
        all_stubs = self.stubs._stub_list; N = len(all_stubs)
        ...
        raw_scores = self.kernel.score_anchor_vs_batch(
            self._instr_rho, self._instr_theta, self._instr_sigma, s_rho, s_theta, s_sigma)
        ...
        for rank, (idx_t, score_t) in enumerate(zip(top_idx, top_vals)):
            idx   = idx_t.item(); stub = all_stubs[idx]
            ...
            directives.append(StubDirective(
                stub_idx=idx, stub_rho=stub.rho, stub_theta=stub.theta,
                stub_sigma=stub.sigma, directive=amplitude * rank_discount))
        return directives
...
class RippleShiftEngine:
    def __init__(self, aniso_kernel: AnisoDirKernel,
                 ...
                 device: torch.device = DEVICE):
        self.kernel = aniso_kernel; self.decay = ripple_decay
        self.scale  = ripple_scale; self.device = device
    ...
    def compute(self, directives: List[StubDirective],
        ...
        for d in directives:
            k_scores = self.kernel.score_anchor_vs_batch(
                d.stub_rho, d.stub_theta, d.stub_sigma, c_rho, c_theta, c_sigma)
        ...
        # ── MINI-THÉBAULT site 1: RippleShiftEngine.compute ─────────────────
        return mini_thebault(ripple * self.scale, gamma=0.15)
...
class IsolatedKernelProductLayer(nn.Module):
    ...
    def forward(self, feat_a, feat_b): return feat_a @ feat_b.T


class IsolatedNystromBuildLayer(nn.Module):
    def __init__(self): super().__init__()

    def forward(self, phi_c, phi_lm) -> torch.Tensor:
        K_cm = phi_c @ phi_lm.T; K_mm = phi_lm @ phi_lm.T
        try:
            ...
            K_mm_inv = torch.eye(phi_lm.shape[0], dtype=phi_c.dtype, device=phi_c.device) * 0.01
...
class IsolatedSynapticSumLayer(nn.Module):
    def __init__(self): super().__init__()
    def forward(self, W, logits): return W @ signed_power(logits, p=1.0)


class IsolatedAnisoGramLayer(nn.Module):
    def __init__(self, aniso_kernel): super().__init__(); self._k = aniso_kernel
    def forward(self, c_rho, c_theta, c_sigma): return self._k.gram_matrix(c_rho, c_theta, c_sigma)


class IsolatedLSHProjectLayer(nn.Module):
    def __init__(self): super().__init__()
    def forward(self, feats, planes): return (feats @ planes.T > 0).int()


class IsolatedCompositionBonusLayer(nn.Module):
    def __init__(self): super().__init__()
    def forward(self, ctx_feat, cand_feats): return (ctx_feat @ cand_feats.T).squeeze(0).clamp(0.0)


class IsolatedSpaghettiBlendLayer(nn.Module):
    def __init__(self): super().__init__()
    def forward(self, a, b, coupling=SPAGHETTI_COUPLING): return mobius_cross_shift(a, b, coupling)
...
class SequentialLayerActivator(nn.Module):
    LAYER_NAMES = [
        "rff_project", "kernel_product", "nystrom_build", "synaptic_sum",
        "aniso_gram", "lsh_project", "composition_bonus", "spaghetti_blend",
    ]

    def __init__(self, rff, aniso_kernel, residual_scale: float = 0.1,
                 device: torch.device = DEVICE):
        super().__init__()
        self.residual_scale = residual_scale; self.device = device; self._tok_step: int = 0
        self.layers = nn.ModuleList([
            IsolatedRFFProjectLayer(rff),
            IsolatedKernelProductLayer(),
            IsolatedNystromBuildLayer(),
            IsolatedSynapticSumLayer(),
            IsolatedAnisoGramLayer(aniso_kernel),
            IsolatedLSHProjectLayer(),
            IsolatedCompositionBonusLayer(),
            IsolatedSpaghettiBlendLayer(),
        ])
        self._num_layers = len(self.layers)

    def tick(self):  self._tok_step += 1
    def reset(self): self._tok_step = 0

    @property
    def active_layer_idx(self) -> int: return self._tok_step % self._num_layers
    @property
    def active_layer_name(self) -> str: return self.LAYER_NAMES[self.active_layer_idx]

    def gate(self, layer_idx: int) -> float:
        return 1.0 if layer_idx == self.active_layer_idx else self.residual_scale

    def rff_features(self, rho, theta, sigma):
        return self.layers[0](rho, theta, sigma) * self.gate(0)

    def kernel_product(self, feat_a, feat_b):
        return self.layers[1](feat_a, feat_b) * self.gate(1)

    def nystrom_build(self, phi_c, phi_lm):
        return self.layers[2](phi_c, phi_lm) * self.gate(2)

    def synaptic_sum(self, W, logits):
        return self.layers[3](W, logits) * self.gate(3)

    def aniso_gram(self, c_rho, c_theta, c_sigma):
        return self.layers[4](c_rho, c_theta, c_sigma) * self.gate(4)

    def lsh_project(self, feats, planes):
        return self.layers[5](feats, planes)

    def composition_bonus(self, ctx_feat, cand_feats):
        return self.layers[6](ctx_feat, cand_feats) * self.gate(6)

    def spaghetti_blend(self, a, b, coupling=SPAGHETTI_COUPLING):
        return self.layers[7](a, b, coupling) * self.gate(7)
...
# ---------------------------------------------------------------------------
# ██  NYSTROM SYNAPTIC MATRIX
# ---------------------------------------------------------------------------
class NystromSynapticMatrix:
    def __init__(self, rff, n_landmarks=RP_NYSTROM_M, top_k=8,
                 device=DEVICE, dtype=torch.float32):
...
class LSHIndex:
    ...
    def build(self, features, vocab, _activator=None):
        self._feats = features; self._vocab = vocab; self._table = {}
        bits = (_activator.lsh_project(features, self.planes) if _activator is not None
                else (features @ self.planes.T > 0).int())
    ...
    def query_candidates(self, q_feat, max_cands=50, _activator=None):
        if self._feats is None: return []
        bits = (_activator.lsh_project(q_feat.to(self.device), self.planes) if _activator is not None
                else (q_feat.to(self.device) @ self.planes.T > 0).int())
        cands: Set[int] = set()
        for b in range(self.n_bands):
            s   = b * self.n_rows
...
# ---------------------------------------------------------------------------
# ██  RANDOM WALK POTENTIAL ENGINE
# ---------------------------------------------------------------------------
class RandomWalkPotentialEngine:
    def __init__(self, n_walks=RP_WALK_STEPS, walk_length=12, restart_p=0.15, device=DEVICE):
        self.n_walks = n_walks; self.walk_length = walk_length
        self.restart_p = restart_p; self.device = device
    ...
    def propagate(self):
        ...
        starts = [self._all_toks[i]
                  for i in _reservoir_sample_indices(len(self._all_toks), 500)]
        for src in starts:
            cur = src
...
class SketchedPDNEngine:
    def __init__(self, n_modes=4, n_samples=200, sigma_pdn=0.25,
                 orbit_weight=0.4, regularity_weight=0.5,
                 device=DEVICE, dtype=torch.float32):
        ...
        self.device = device; self.dtype = dtype
        self.n_star = 4; self.power_spectrum: Dict[int, float] = {}
    ...
    def pdn_logit_bonus(self, window_rho, window_theta, c_rho, c_theta, current_orbit):
        ...
        def _n(x):
            std = x.std()
            return (x - x.mean()) / (1e-8 + std) if std.item() > 1e-8 else x - x.mean()
...
def _hyp_dist(x1, y1, x2, y2, eps=1e-8):
    ...
    return math.acosh(max(z, eps + 1.0))
...
class BolyaiTokenGeometryRP:
    ...
    def register(self, token, freq, index, max_freq, vocab_size):
        ...
        r   = 0.12 * math.sqrt(max(f, 1e-9)); ang = 2.0 * math.pi * k
        self._vecs[token] = (r * math.cos(ang), r * math.sin(ang))
    ...
    def composed_triple(self, t1, t2):
        ...
        eu    = 1.0 - min(math.sqrt(y*y + x*x), 1.0 - 1e-8)
        hyp_r = 2.0 * torch.atanh(torch.tensor(eu))
        rho   = torch.tanh(0.5 * torch.tensor(hyp_r))
        theta = math.atan2(y, x) % math.pi
        sigma = 2.0 / max(1.0 - eu*eu, 1e-8)
...
def compute_transitive_triples_rp(geo, cands, w1, w2, device=DEVICE, dtype=torch.float32):
    p1x, p1y = geo._vec(w1); p2x, p2y = geo._vec(w2)
    rl, tl, sl = [], [], []
    for c in cands:
        pcx, pcy = geo._vec(c)
...
# ---------------------------------------------------------------------------
# ██  CROSS SYNAPTIC NEURON SUM  (mini-thébault sites 4 & 5)
# ---------------------------------------------------------------------------
class RPCrossSynapticNeuronSum:
    def __init__(self, rff, syn_weight=0.4, trans_weight=0.6, syn_k=8,
                 device=DEVICE, dtype=torch.float32):
    ...
    @torch.no_grad()
    def synaptic_sum(self, logits, c_rho, c_theta, c_sigma, _activator=None):
        W   = self._nystrom.build(c_rho, c_theta, c_sigma, _activator=_activator)
        raw = (W @ signed_power(logits, p=1.0)) if _activator is None \
              else _activator.synaptic_sum(W, logits)
        # ── MINI-THÉBAULT site 4 ────────────────────────────────────────────
        return mini_thebault(layer_norm_array(raw), gamma=0.10)

    @torch.no_grad()
    def transitive_bonus(self, c_rho_t, c_theta_t, c_sigma_t,
                         ctx_rho, ctx_theta, ctx_sigma, _activator=None):
        feat_a = self.rff.features(
            ...
            _activator=_activator)
        feat_b = self.rff.features(c_rho_t, c_theta_t, c_sigma_t, _activator=_activator)
        raw    = feat_a @ feat_b.T
        if _activator is not None:
            raw = raw * _activator.gate(1)
        # ── MINI-THÉBAULT site 5 ────────────────────────────────────────────
        return mini_thebault(layer_norm_array(raw.squeeze(0).clamp(0.0)), gamma=0.10)
    ...
    def forward(self, logits, c_rho, c_theta, c_sigma,
        ...
        tb    = self.transitive_bonus(c_rho_t, c_theta_t, c_sigma_t,
                                      ctx_rho, ctx_theta, ctx_sigma, _activator=_activator)
...
class GeometryConstructor:
    def __init__(self, device=DEVICE, dtype=torch.float32, rff_dim=RP_RFF_DIM):
        self.device = device; self.dtype = dtype; self.rff_dim = rff_dim

    def construct(self, raw_freq: Dict[str, float],
                  vocab: List[str]) -> Tuple[BolyaiTokenGeometryRP, RandomFourierFeatures]:
        ...
        if not vocab:
            ...
            return geo, rff
        V = len(vocab); max_freq = max(raw_freq.values(), default=1e-9)
...
def tokenize(text: str) -> List[str]:
    ...
    for w in text.split():
        ...
        if wc:
            out.append(f"[{wc.upper()}]" if wc in STOP_WORDS_COG else wc)
    return out
...
def detokenize(tokens: List[str]) -> str:
    ...
    for t in tokens:
        ...
        if t in COGNITIVE_TOKENS:
            raw = t.strip("[]").lower()
            res.append(raw.capitalize()
                       if not res or res[-1].endswith(('.', '!', '?')) else raw)
        else:
            res.append(t.capitalize()
                       if not res or res[-1].endswith(('.', '!', '?')) else t)
    out = " ".join(res).strip()
...
class RPCompositionLM:
    ...
    def next_dist(self, w1, w2):
        if (w1, w2) in self.heads:
            return self.heads[(w1, w2)], self._head_probs[(w1, w2)]
        agg = {}
        for (_, _, w3), _ in self.tri_raw.items():
            agg[w3] = self.cms.query(w3) + agg.get(w3, 0)
        cands_all = list(agg.keys())
        cands     = [cands_all[i] for i in _reservoir_sample_indices(
            len(cands_all), min(RP_RESERVOIR_K * 4, len(cands_all)))]
        total = sum(agg.get(c, 1e-4) for c in cands); V = 1 + len(self.vocab)
        return cands, torch.tensor(
            [(self.BASAL_K + agg.get(c, 1e-4)) / (self.BASAL_K * V + total) for c in cands],
            dtype=torch.float32, device=self.device)

    def composition_logit_bonus(self, w1, w2, c_rho, c_sigma, _activator=None):
...
class RPMRVFilter:
    ...
    def prime(self, vocab, geo):
        ...
        if geo._feat_t is not None:
            self._vocab_feats = geo._feat_t[geo.tok_indices(vocab)]
        else:
            triples = [geo.triple_fast(v) for v in vocab]
        ...
        self._lsh.build(self._vocab_feats, vocab)
    ...
    def mrv_scores_batched(self, c_rho, c_sigma, kernels=None):
        ...
        for i in range(C):
            domain[i] = float(len(self._lsh.query_candidates(c_feats[i], max_cands=30)))
...
# ---------------------------------------------------------------------------
# ██  COT STUB LIBRARY  (mini-thébault site 7 lives in RPCoTReasoningEngine)
# ---------------------------------------------------------------------------
...
class ContextualStub:
    stub_type: str; tokens: List[str]; rho: float; theta: float
    sigma: float; weight: float; label: str = ""
...
class RPCoTStubLibrary:
    def __init__(self, rff, rho_threshold=0.20, n_theta_bins=8, min_bin_size=2,
                 device=DEVICE, dtype=torch.float32):
        self.rff = rff; self.rho_threshold = rho_threshold
        self.n_theta_bins = n_theta_bins; self.min_bin_size = min_bin_size
        self.device = device; self.dtype = dtype
    ...
    def build(self, geo, lm_vocab, raw_freq):
        ...
        for stub_type, bucket in zip(_STUB_SEQUENCE[:4],
                                     [bridges[:q], bridges[q:2*q],
                                      bridges[2*q:3*q], bridges[3*q:]]):
    ...
    def _make_stub(self, stub_type, bi, sub_idx, members):
        ...
        self.stubs[stub_type].append(ContextualStub(
            ...
            theta=math.atan2(sin_m, cos_m) % math.pi,
            sigma=sum(sigmas) / len(sigmas),
            weight=sum(m[2] for m in members),
            label=f"[{stub_type}|bin{bi}|{'hi' if sub_idx else 'lo'}-σ] {' '.join(toks[:3])}…"))
    ...
    def _rebuild_lsh(self):
        ...
        self._stub_lsh = LSHIndex(feature_dim=fd, n_bands=RP_LSH_BANDS,
    ...
    def best_stub(self, stub_type, ctx_rho, ctx_theta, ctx_sigma,
        ...
        if pdn_engine is not None:
            scores = 0.3 * pdn_engine.orbit_bonus(pdn_orbit, ct) + scores
        return cands[int(scores.argmax().item())]

    @torch.no_grad()
    def stub_kernel(self, stub, c_rho, c_theta, c_sigma, kernels=None):
        return self.rff.kernel_scalar(
            stub.rho, stub.theta, stub.sigma, c_rho, c_theta, c_sigma).clamp(0.0)


# ---------------------------------------------------------------------------
# ██  COT REASONING ENGINE  (mini-thébault site 7)
# ---------------------------------------------------------------------------
class RPCoTReasoningEngine:
    def __init__(self, stub_library, kernels, pdn_engine,
                 n_hops=3, tokens_per_hop=8, stub_logit_scale=0.9,
                 device=DEVICE, dtype=torch.float32):
    ...
    def plan_chain(self, seed_tokens, geo, pdn_orbit=0) -> CoTTrace:
        ...
        for i, stype in enumerate(hops[:self.n_hops]):
            stub = self.stubs.best_stub(stype, ctx_rho, ctx_theta, ctx_sigma, self.kernels,
                                         pdn_orbit=(i + pdn_orbit) % self.pdn.n_star,
                                         pdn_engine=self.pdn)
            if stub is None: continue
            k = self.stubs.stub_kernel(
                stub,
        ...
        self._conclusion_stub = self.stubs.best_stub(
            ...
            pdn_orbit=(self.n_hops + pdn_orbit) % self.pdn.n_star, pdn_engine=self.pdn)
    ...
    def active_bonus(self, c_rho, c_theta, c_sigma, token_position, total_tokens):
        ...
        # ── MINI-THÉBAULT site 7 ────────────────────────────────────────────
        return mini_thebault(raw * self.stub_logit_scale, gamma=0.11)
...
class synthetic_reasonMandateProcessor:
    def __init__(self):
        self.AIEthics  = ["maintain objective reality", "identify as software",
                          "do not claim physical form"]
        self.AIMandates = ["state facts", "deduce logically", "avoid metaphors",
                           "remain objective"]
        self.mandate_vocabulary = {
            "exist": "fact", "identity": "algorithm", "am": "software",
            "banana": "avoid", "basket": "avoid", "metaphor": "prevent",
            "physical": "deny", "logic": "deduce"}
...
class ChunkedSumEngine:
    ...
    def push(self, triple, pos_norm):
        self._buf[self._ptr] = torch.tensor(
            [triple.rho, triple.theta / math.pi, triple.sigma, pos_norm],
            dtype=self.dtype, device=self.device)
        self._ptr = (1 + self._ptr) % self.window_size
        self._count = min(1 + self._count, self.window_size)
    ...
    def chunk_signature(self):
        ...
        if pad > 0:
            w = torch.cat([w, torch.zeros(pad, VEC_DIM, dtype=self.dtype, device=self.device)])
...
# ---------------------------------------------------------------------------
# ██  ISOMORPHIC SYNTAX STACKER  (mini-thébault site 6)
# ---------------------------------------------------------------------------
class IsomorphicSyntaxStacker:
    ...
    def add(self, tokens, geo, text):
        ...
        if not clean: return
        rhos = torch.tensor([geo.triple_fast(t).rho   for t in clean],
                             dtype=self.dtype, device=self.device)
        sigs = torch.tensor([geo.triple_fast(t).sigma for t in clean],
                             dtype=self.dtype, device=self.device)
    ...
    def ranked_anchors(self, current_tokens, geo, kernels):
        ...
        clean = [t for t in current_tokens
                 if t not in PUNCT_TOKENS and t not in COGNITIVE_TOKENS]
        if not clean: return []
        cr = torch.tensor([geo.triple_fast(t).rho   for t in clean],
                           dtype=self.dtype, device=self.device)
        cs = torch.tensor([geo.triple_fast(t).sigma for t in clean],
                           dtype=self.dtype, device=self.device)
    ...
    def syntax_echo_bonus(self, c_rho, c_sigma, current_tokens, geo, kernels, echo_weight=0.5):
        ...
        # ── MINI-THÉBAULT site 6 ────────────────────────────────────────────
        return mini_thebault(bonuses * echo_weight, gamma=0.13)


# ---------------------------------------------------------------------------
# ██  GEOMETRIC TEMP SCALER / DNN PIPELINE
# ---------------------------------------------------------------------------
...
class DNNArrayPipeline:
    def __init__(self, device=DEVICE, dtype=torch.float32):
        self.device = device; self.dtype = dtype
        self.temp_scaler = GeometricTempScaler(lambda_temp=1.0)
...
# ---------------------------------------------------------------------------
# ██  NL MICRO SIMULATION + NANOWIRE CANVAS
# ---------------------------------------------------------------------------
...
def sharpen_rho_trend(rho):    return torch.sigmoid((rho - rho.mean()) * 5.0)
def chromatic_theta_trend(th): return 1.0 + 0.5 * torch.sin(th * 3.0)
def glow_sigma_trend(sigma):   return torch.exp(sigma)


class NanowireStream:
    def __init__(self, name: str, aesthetic_func):
        self.name = name; self.aesthetic_func = aesthetic_func
...
class NanowireCanvas:
    def __init__(self):
        self.rho_brush = self.theta_brush = self.sigma_brush = self.art_tensor = None

    def update_art(self, numpy_img):
        if numpy_img is None: self.art_tensor = None; return
        if isinstance(numpy_img, dict):
            numpy_img = (numpy_img.get('composite') or numpy_img.get('image')
                         or numpy_img.get('background'))
        if isinstance(numpy_img, np.ndarray) and len(numpy_img.shape) == 3 and numpy_img.shape[2] >= 3:
            self.art_tensor = (torch.tensor(numpy_img[:, :, :3], dtype=torch.float32)
                               .permute(2, 0, 1) / 255.0)
        else:
            self.art_tensor = None

    def equip_brushes(self, rho_stream, theta_stream, sigma_stream):
        self.rho_brush = rho_stream; self.theta_brush = theta_stream
    ...
    def _apply_art_trend(self, vector, channel_idx):
        ...
        norm_v = (vector - v_min) / (v_max - v_min) if v_max > v_min else torch.zeros_like(vector)
        x_coords    = (norm_v * (W - 1)).long().clamp(0, W - 1)
        col_intensity = self.art_tensor[channel_idx].to(vector.device).mean(dim=0)
        return 0.5 + col_intensity[x_coords] * 10.5
...
# ---------------------------------------------------------------------------
# ██  TOKEN STEP TRACE + PROPOSITIONAL ENGINE
# ---------------------------------------------------------------------------
@dataclass
...
class PropositionalSurjectionEngine:
    ...
    def surject_sentence(self, tokens):
        ...
        triples    = [self.geo.triple_fast(t) for t in clean]
        statements = []
        for i in range(len(clean) - 2):
            w1, w2, w3 = clean[i], clean[i+1], clean[i+2]
...
class RPInstructionDistribution:
    def __init__(self, geo, kernels, lm, device=DEVICE, dtype=torch.float32,
        ...
        self.instr_toks = []; self.instr_freq = {}
        self.instr_centroid = None; self.base_dist_t = None
    ...
    def set_instruction(self, instruction_text):
        raw = tokenize(instruction_text)
        self.instr_toks = [t for t in raw
                           if t not in PUNCT_TOKENS and t not in COGNITIVE_TOKENS]
        if not self.instr_toks:
            self.base_dist_t = None; self.instr_centroid = None; return
        if (self.geo._rho_t is None or self.geo._theta_t is None or
                self.geo._rho_t.shape[0] != len(self.lm.vocab)):
...
class FittedLineRegression(nn.Module):
    ...
    def feature_report(self) -> str:
        lines = ["  Fitted Line Feature Weights (V18-RP-ANISO-RIPPLE-SPAGHETTI-CARDAN+THÉBAULT):"]
...
class RPWalker:
    def __init__(self, geo, kernels, lm, orbit, rw_graph, synth, mrv_filter,
        ...
        self._activator = SequentialLayerActivator(
            rff=rff, aniso_kernel=self._aniso_kernel,
            residual_scale=0.10, device=device)
    ...
    def begin_sentence(self, seed_tokens=None, total_tokens=40) -> CoTTrace:
        ...
        self._ooi_tracker.reset(); self._activator.reset()
    ...
    def _ripple_features(self, c_rho, c_theta, c_sigma):
    ...
    def _extract_features(self, C,
                          ...
                          ooi_affinity, inter_repulsion, ripple_shift,
                          spaghetti_tangle_norm,
                          cardan_aperture_scores, mirror_signal) -> torch.Tensor:
        def _safe(t):
        ...
        return torch.stack([
            ...
            _safe(sorted_impulse), _safe(ooi_affinity), _safe(-inter_repulsion),
            _safe(ripple_shift), _safe(spag_col),
            _safe(cardan_aperture_scores), _safe(mirror_signal),
        ], dim=-1)
    ...
    def _paragraph_duplication(self, cands):
        ...
        for cand in cands:
            ...
            for para in self._recent_paragraphs:
                max_k = min(len(trial), len(para), PARA_DUP_MATCH_CAP); hit = 0
    ...
    def walk_probs(self, w1, w2, temp=1.4,
        ...
        C = len(cands)
        if C == 0: return cands, base_probs
        ...
        syn_norm   = z_syn_raw.norm().item(); trans_norm = t_bon_raw.norm().item()
        self._csns_syn_norms.append(syn_norm); self._csns_trans_norms.append(trans_norm)
        ...
        def _znorm(t):
            s = t.std()
            return (t - t.mean()) / (1e-8 + s) if s.item() > 1e-8 else t - t.mean()
        ooi_aff_norm = _znorm(ooi_affinity); rep_norm = _znorm(inter_repulsion)
        ...
        raw_logits = logits_enriched + spaghetti_logits + mandate_boost + punct_bias
        ...
        features = self._extract_features(
            ...
            spaghetti_tangle_norm, aperture_scores_full, mirror_sig)
        ...
        # ── MIRRORED SAWTOOTH CLIP (replaces the silent no-op torch.clip) ───
        final_probs = mirrored_sawtooth_clip(final_probs, period=0.1)
        return cands, final_probs

    def record_step_trace(self, step, chosen, cands, final_probs, and_weight):
        try:
            idx = cands.index(chosen); p_and = final_probs[idx].item()
        except:
            idx = 0; p_and = 0.0
        ...
        if hasattr(self, '_pending_walk_logits') and self._pending_walk_logits is not None:
            log_walk = self._dnn.log_forward(
                self._pending_walk_logits, self._pending_crho,
                self._pending_ctheta, self._pending_csigma, temp=1.0)
            p_walk = log_walk[idx].exp().item()
        else:
            p_walk = 0.0
        source = ("instr" if p_instr > p_walk * 1.5
                  else "walker" if p_walk > p_instr * 1.5 else "AND")
        ...
        trace = TokenStepTrace(
            ...
            ripple_mean=self._pending_ripple_mean, n_directives=self._pending_n_directives,
    ...
    def algo_report(self) -> str:
        ...
        return "\n".join([
            "V18-RP-ANISO-RIPPLE-SPAGHETTI-CARDAN + MINI-THÉBAULT",
            "",
            "MINI-THÉBAULT SITES:",
            "  1. RippleShiftEngine.compute           γ=0.15",
            "  2. SentenceOOITracker.ooi_affinity     γ=0.12",
            "  3. SentenceOOITracker.inter_repulsion  γ=0.12",
            "  4. RPCrossSynapticNeuronSum.synaptic   γ=0.10",
            "  5. RPCrossSynapticNeuronSum.transitive γ=0.10",
            "  6. IsomorphicSyntaxStacker.echo_bonus  γ=0.13",
            "  7. RPCoTReasoningEngine.active_bonus   γ=0.11",
            "  8. SpaghettiMixer.blend                γ=0.10",
            "",
...
# ---------------------------------------------------------------------------
# ██  DATASET BASELINE + AUTOMORPHISMS
# ---------------------------------------------------------------------------
...
def pairwise_sort_unlink(cands, probs):
    ...
    return unlinked_cands, torch.tensor(unlinked_probs, dtype=probs.dtype, device=probs.device)
...
def apply_bilinear_lateral_automorphism(cands, probs, lateral_coupling=-0.3):
    ...
    return unlinked_cands, new_probs / new_probs.sum()
    ...
    for sent_idx in range(num_sentences):
        if sent_idx == 0:
            ...
            if len(next_unspoken_utterance) >= 2:
                ...
                w1_start, w2_start = random.choice(head_list); init_toks = []
        ...
        for attempt in range(MAX_REDO):
            ...
            for step in range(12 + tokens_per_sent):
                ...
                # inner loop was a bug in original — fixed to single application
                cands, probs = apply_bilinear_lateral_automorphism(
                    cands, probs, lateral_coupling=-0.35)
...
class V18RPEngine:
    ...
    def train(self, corpus_text: str):
        ...
        fwd_dist    = RPInstructionDistribution(self.geo, self.kernels, self.lm, device=self.device)
        ...
        self.walker = RPWalker(
            ...
            self.rff, cardan=self.cardan, device=self.device,
        ...
        print("[V18-RP-CARDAN+THÉBAULT] Engine ready.")
    ...
    def train_fitted_line(self, corpus_text="", epochs=200, lr=3e-4, max_steps=50000):
        ...
        if len(tokens) < 10:
            print("[FittedLine] Corpus too short!"); return self.walker.fitted_model
...
def engine_status() -> str:
    ...
    return (f"✓ READY  |  V18RPEngine + mini-Thébault\n"
            f"   Vocab: {vocab_sz:,}   Trigrams: {tri_sz:,}\n"
            f"   Device: {getattr(engine, 'device', '?')}")
...
def gui_init(mode, filein,
    ...
    try:
        engine = V18RPEngine(
            ...
            spaghetti_coupling=float(spag_c), cardan_aperture_k=int(cardan_k),
            mirror_alpha=float(mirror_a), cardan_logit_weight=float(cardan_lw))

        if mode == "Text file":
            if filein is None: return "No file uploaded"
            ...
            if hfmax and int(hfmax) > 0: rows = min(rows, int(hfmax))
            text = "\n".join(str(ds[i].get(field, "")) for i in range(rows))
        else:
            return "Unknown mode"

        engine.train(text)
        vocab_sz = len(engine.lm.vocab); tri_sz = len(engine.lm.tri_raw)
        pdn_rep  = engine.pdn.theorem_bridge_report()
        card_rep = engine.cardan.rotation_report() if engine.cardan else ""
        return (f"ENGINE READY - V18-RP-CARDAN + mini-Thébault\n"
                f"Vocab: {vocab_sz:,}   Trigrams: {tri_sz:,}   Device: {engine.device}\n"
...
def gui_fitline(epochs, lr, maxsteps):
    if not _ready(): return "Engine not ready, run Init/Train first"
    import traceback
    try:
        model = engine.train_fitted_line(
            epochs=int(epochs), lr=float(lr), max_steps=int(maxsteps))
...
def gui_load_fitted(path):
    if not _ready(): return "Engine not ready"
    import traceback
    try:
    ...
    try:
        ...
        if not props and hasattr(engine.walker, '_prop_traces') and engine.walker._prop_traces:
            ...
            for sent_idx, stmts in engine.walker._prop_traces[-4:]:
                for s in stmts: prop_lines.append(s.render())
            props = "\n".join(prop_lines)
...
def gui_auto_load():
    global LATEST_AUTONOMIC_VAL
    if not os.path.exists(AUTONOMIC_FILE): return "No save file found", 1.0
    try:
    ...
    except Exception as e: return str(e), 1.0
...
def build_app():
    with gr.Blocks(title="V18-RP-CARDAN+THÉBAULT", theme=gr.themes.Soft()) as demo:
        gr.Markdown(
            "# V18-RP-ANISO-RIPPLE-SPAGHETTI-CARDAN + mini-Thébault\n"
            "**Cardan Grille Isomorphisms · Mirrored Instructions · "
            "Möbius Spaghetti Router · Thébault curvature correction at 8 sites**")

        with gr.Tab("Init / Train"):
            mode   = gr.Radio(["Text file", "HuggingFace"], value="Text file", label="Corpus source")
            ...
            init_btn.click(
                gui_init,
                inputs=[mode, filein, hfname, hfconfig, hfsplit, hffield, hfportion, hfmax,
                        syn_w, trans_w, syn_k, rff_dim, nystrom,
                        ooi_w, rep_w, rpl_w, rpl_k, spag_c, cardan_k, mirror_a, cardan_lw],
                outputs=init_out)
        ...
        with gr.Tab("Generate"):
            with gr.Row():
                ...
                instr_txt = gr.Textbox(label="Instruction",
                                       value="You are a computational algorithm.", lines=2)
            with gr.Row():
            ...
            gen_btn.click(
                gui_generate,
                inputs=[seed_txt, instr_txt, nsents, tokpersent, and_w, temp, show_tr, artimg],
        ...
        with gr.Tab("Save / Load"):
            ...
            with gr.Row():
                gr.Button("Save autonomic").click(
                    gui_auto_save, outputs=gr.Textbox(label="Save status"))
                auto_load_out = gr.Textbox(label="Load status")
                auto_val_out  = gr.Slider(0.0, 1.0, 1.0, interactive=False, label="Loaded value")
                gr.Button("Load autonomic").click(
                    gui_auto_load, outputs=[auto_load_out, auto_val_out])
    return demo


if __name__ == "__main__":
    app = build_app()
    app.launch(server_name="127.0.0.1", share=False, show_error=True, inbrowser=True)